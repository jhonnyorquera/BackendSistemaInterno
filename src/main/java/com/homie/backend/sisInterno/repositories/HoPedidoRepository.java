...
public interface HoPedidoRepository extends CrudRepository<HoPedido, String> {
	...
	@Query(value = "select ped.pe_codigo, ped.pe_valor, COALESCE((select sum(pa.pp_valor) from ho_pedido_pagos "
			+ "where ped.pe_codigo=ho_pedido_pagos.ho_pedido and ho_pedido_pagos.pp_estado = true"
			+ " group by ped.pe_codigo),0)" + " ,cl.cl_nombre, ped.pe_status_pago"
